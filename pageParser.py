...
def getFlows(stationId):
	 ...
	 return np.array([(datetime.strptime(ts.groups()[0], '%d/%m/%Y %H:%M'), np.double(ts.groups()[1])) for ts in re.finditer("title=.?(.{16}) Valeur .* = (\d+\.\d+)", flowGraphText)])
...
def getHeights(stationId):
	 ...
	 return np.array([(datetime.strptime(ts.groups()[0], '%d/%m/%Y %H:%M'), np.double(ts.groups()[1])) for ts in re.finditer("title=.?(.{16}) Valeur .* = (\d+\.\d+)", heightGraphText)])
